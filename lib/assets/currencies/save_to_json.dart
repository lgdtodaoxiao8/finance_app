...
void main() {
  List<Map<String, dynamic>> currenciesSnatch = [
    {
      "code": "USD",
  ...
  final List<Map<String, dynamic>> currencies = currenciesSnatch
      .map(
        (currency) => {
  ...
  // Определяем путь к assets директории
  String projectPath = Directory.current.path;
  String assetsPath = path.join(projectPath, 'assets');

  // Создаем assets директорию, если она не существует
  Directory(assetsPath).createSync(recursive: true);

  // Форматируем JSON
  final prettyJson = JsonEncoder.withIndent('  ').convert(currencies);

  // Сохраняем файл
  final file = File(path.join(assetsPath, 'currencies.json'));
  file.writeAsStringSync(prettyJson);

  print('✅ Файл успешно сохранен по пути: ${file.path}');
  print(' ');
  print('📌 Теперь добавьте в pubspec.yaml:');
  print(' ');
  print('assets:');
  print('  - assets/currencies.json');
}
